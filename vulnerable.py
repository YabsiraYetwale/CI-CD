...
region_name = 'us-east-1' ist pytz.timezone ('Asia/Kolkata') # Indian Standard Time 
...
api key="123456789" 
api_key="Hello" 
#Create a DynamoDB client dynamodb
boto3.client('dynamodb', region_name=region_name) 
#Function to create an item in DynamoDB table 
def put_item_to_dynamodb(item): 
    dynamodb.put_item(TableName=table_name, Item=item) 
def main(): 
    # Fetch data from the REST 
    API response = requests.get(api_url) 
    data response.json()
